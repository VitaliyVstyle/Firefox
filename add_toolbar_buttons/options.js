function SaveForm() {
    ...
    browser.addToolbarButtons.setPref(arr);
}
function initOptions() {
    ...
    for (let l of locales)
        l[(l.value === undefined || l.localName === "option") ? "textContent" : "value"] = browser.i18n.getMessage(`${l.dataset.locale}`);
    var inputs = document.querySelectorAll("[data-pref]"), arr = [];
    for (let i of inputs)
        arr.push(i.dataset.pref);
    browser.addToolbarButtons.getPref(arr).then(res => {
        res.forEach(nv => {
